      ...
      if (partition.intersects(bounds)) {
        for (const auto &c : partition.children) {
          if (c.intersects(bounds)) {
            const auto it = previouslyVisible.find(c.layout->layout.component);
            if (it == previouslyVisible.end()
                || it->second.checkedOutViews.empty()) {
              return true;
            }
          }
        }
...
public:
  ...
  void unmountAllChildren(void)
  {
    ...
    for (const auto &mountResult : _mountedComponentResultMap) {
      ...
      for (const auto &checkout : mountResult.second.checkedOutViews) {
        [checkout.view setHidden:YES];
      }
                              ...
                              supercomponent:(CKComponent *)supercomponent
{
  ...
  if (children && !children->empty()) {
    const auto pair = addListenerForParentScrollView(self, context.viewManager->view);
    _scrollListeningToken = pair.first;
    _mountController = {
      _component,
      self,
      context.viewManager->view,
      pair.second,
